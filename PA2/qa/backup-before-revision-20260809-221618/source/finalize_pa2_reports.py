...
def cover(doc: Document, title: str, subtitle: str):
    m.add_cover(doc, title, subtitle, "FINAL - simulated-study evidence explicitly labeled")
...
def build_user_research() -> Path:
    ...
    portrait(doc)
    doc.add_heading("10. Evidence and course-source summary", 1)
...
def build_user_analysis() -> Path:
    ...
    doc.add_heading("8. Course-source application", 1)
    doc.add_paragraph(COURSE)
...
def build_project_proposal() -> Path:
    ...
    comparison = [
        ...
        ["C-A2 Conversational Coach", "4", "5", "4", "3", "2", "3", "3.75", "Flexible but consistency, service fallback, and validation are harder."],
    ...
    doc.add_heading("7. Course-source application", 1)
    doc.add_paragraph(COURSE)
...
def build_use_cases() -> Path:
    ...
    doc.add_heading("4. Course-source application", 1)
    doc.add_paragraph(COURSE)
...
- Peer Review feedback remains unavailable in the existing artifact and the existing Peer Review PDF is intentionally unchanged.
