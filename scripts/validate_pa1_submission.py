...
class Validator:
    ...
    def check_weekly(self, text: str, label: str) -> None:
        errors: list[str] = []
        if f"{self.group_id}-PA1 Weekly Scrum Meeting Minutes" not in text:
            errors.append("exact title missing")
        required = (
            "Process Overview", "RUP + Scrum", "Inception", "Elaboration", "Construction", "Transition",
            "One sprint is fixed for 2 weeks", "Google Docs and Drive Structure", "Team roster",
            "Meeting schedule", "Sprint Planning Meeting Minutes", "Weekly Scrum 1 Meeting Minutes",
            "Weekly Scrum 2 Meeting Minutes", "Sprint Review and Retrospective Meeting Minutes",
            "Weekly scrum meeting minutes template", "Sprint planning meeting minutes template",
            "Sprint review meeting minutes template", "Workload matrix", "Final submission checklist",
        )
        errors.extend(f"missing {term}" for term in required if term.casefold() not in text.casefold())
        ordered_sections = (
            "Section 1: Process Overview", "Section 2: Google Docs and Drive Structure", "Section 3: Team roster", "Section 4: Meeting schedule",
            "Section 5: Sprint Planning Meeting Minutes", "Section 6: Weekly Scrum 1 Meeting Minutes", "Section 7: Weekly Scrum 2 Meeting Minutes",
            "Section 8: Sprint Review and Retrospective Meeting Minutes", "Section 9: Workload matrix", "Section 10: Final submission checklist",
        )
        ...
        for link_label in ("Weekly Scrum Google Docs link", "Sprint Planning Google Docs link", "Sprint Review Google Docs link", "Google Drive README link", "GitHub repo link", "Moodle zip artifact name"):
        ...
        if text.count("25%") != 4:
            errors.append(f"workload matrix must contain four 25% entries (found {text.count('25%')})")
        if "equal ownership".casefold() not in text.casefold():
            errors.append("equal ownership explanation missing")
        markers = (
        ...
        expected_schedule = (
            ("2026-06-10", "20:00 to 20:45"),
            ("2026-06-14", "20:00 to 20:30"),
            ("2026-06-19", "20:00 to 20:30"),
            ("2026-06-22", "20:00 to 20:50"),
        )
        ...
        for index, marker in enumerate(markers):
            ...
            if index == 0:
                for field in ("Sprint objective", "Product decisions", "Priority tasks", "User stories or use-case parts selected for PA1", "Task assignment by member"):
                    if field.casefold() not in block.casefold():
                        errors.append(f"{marker}: missing {field}")
            if index == 3:
                for field in ("What went well", "What went wrong", "What problems occurred", "What caused the problems", "What the team will do differently in the next sprint", "Lessons learned", "Final PA1 packaging decision"):
                    if field.casefold() not in block.casefold():
                        errors.append(f"{marker}: missing {field}")
            if index in (1, 2):
                ...
                if label == "PDF":
                    if not all(block.casefold().count(field.casefold()) >= 4 for field in exact_fields):
                        errors.append(f"{marker}: PDF extraction does not preserve four per-member Scrum field sets")
                    if not all(name in block for name, _ in MEMBERS):
                        errors.append(f"{marker}: PDF extraction is missing one or more members")
                    continue
                tables = markdown_tables(block)
                table_ok = any(
                    all(field.casefold() in [cell.casefold() for cell in header] for field in exact_fields)
                    and all(any(any(name in cell for cell in row) for row in rows) for name, _ in MEMBERS)
                    for header, rows in tables
                )
                if not table_ok:
                    for name, _ in MEMBERS:
                        member_pos = block.find(name)
                        if member_pos < 0:
                            errors.append(f"{marker}: missing member {name}")
                            continue
                        next_positions = [block.find(other, member_pos + len(name)) for other, _ in MEMBERS]
                        next_positions = [pos for pos in next_positions if pos >= 0]
                        member_block = block[member_pos : min(next_positions) if next_positions else len(block)]
                        for field in exact_fields:
                            if field.casefold() not in member_block.casefold():
                                errors.append(f"{marker}/{name}: missing {field}")
